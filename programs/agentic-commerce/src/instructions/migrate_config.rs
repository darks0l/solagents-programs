...
/// New fields (paused, pending_admin) are appended after bump,
...
pub struct MigrateConfig<'info> {
    ...
    /// CHECK: We validate seeds and admin manually to avoid deserialization failure
...
pub fn handler(ctx: Context<MigrateConfig>) -> Result<()> {
    ...
    // Validate: read admin pubkey from offset 8 (first field after discriminator)
    require!(data.len() >= 40, CommerceError::Overflow);
    let stored_admin = Pubkey::try_from(&data[8..40]).map_err(|_| CommerceError::Overflow)?;
    require!(stored_admin == ctx.accounts.admin.key(), CommerceError::UnauthorizedClient);

    let new_size = PlatformConfig::SIZE;
    ...
    // Realloc — zero-fills new bytes (paused=false, pending_admin=default)
